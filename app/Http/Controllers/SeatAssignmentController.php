...
class SeatAssignmentController extends Controller
{
    ...
    public function index()
    {
        ...
        */
        ...
        $morningSeats = SeatAssignment::where('library_id', $libraryId)
            ->where('status', 'Active')
        ...
        */
        ...
        $eveningSeats = SeatAssignment::where('library_id', $libraryId)
            ->where('status', 'Active')
            ->whereNull('released_date')
            ->whereHas('membership.plan', function ($q) {
        ...
        */
        ...
        $fullDaySeats = SeatAssignment::where('library_id', $libraryId)
            ->where('status', 'Active')
            ->whereNull('released_date')
            ->whereHas('membership.plan', function ($q) {
...
$students = Student::where('library_id', $libraryId)
    ->where('status', 'Active')
    ->whereDoesntHave('seatAssignments', function ($query) use ($libraryId) {
...
public function create(Request $request)
{
    ...
    */
    ...
    if ($request->filled('student_id')) {
        ...
        $selectedStudent = Student::where('library_id', $libraryId)
        ...
        */
        ...
        $seats = Seat::where('library_id', $libraryId)
            ->with([
                'room',
...
public function release(SeatAssignment $seatAssignment)
{
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Assignment Must Belong To Current Library
    |--------------------------------------------------------------------------
    */

    abort_unless(
        $seatAssignment->library_id === $libraryId,
        404
    );
...
public function updateChange(
    ...
    */
    ...
    $newSeat = Seat::where('library_id', $libraryId)
        ->with([
            'activeAssignments.membership.plan',
        ])
        ->findOrFail(
